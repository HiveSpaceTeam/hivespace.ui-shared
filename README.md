...
- Pushing a tag like `v1.1.2` also triggers publish.
...
If current version is `1.0.9` and next is `1.0.10`:
...
If you want publish to happen only from tags (safer release control), update workflow triggers to remove `branches: [master]` and keep tag + manual only.
